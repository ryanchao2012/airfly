...
PACKAGE_VERSION := $(lastword $(shell $(POETRY) version))
...
sync: install  ## Sync project metadata with pyproject.toml
	$(POETRY) run inv sync-meta


install: .venv  ## Install dependencies, Options:
##     INSTALL_OPT: options applied to "poetry install", default: $(INSTALL_OPT)
	$(info $(shell tput setaf 2)* Install dependencies $(shell tput sgr0))
	$(POETRY) install $(INSTALL_OPT)
